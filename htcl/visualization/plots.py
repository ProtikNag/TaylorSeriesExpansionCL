...
CHANGES:
- Fixed heatmap annotation issue (all rows now show annotations)
- Added taskwise variance/std plot
- Proper directory structure for saving (plots/png, plots/svg)
"""
...
def save_figure(fig: plt.Figure, name: str, output_dir: str = "./results"):
    """Save figure in both PNG and SVG formats to proper subdirectories."""
...
def plot_forgetting_comparison(
    ...
    forgetting_values = []
    colors = [COLORS["er"], COLORS["htcl_2"], COLORS["htcl_3"], COLORS["htcl_4"], COLORS["htcl_5"]]
...
# =============================================================================
# NEW: Taskwise Variance/Std Plot
# =============================================================================

def plot_taskwise_variance(
    results_list: List[Dict[str, Any]],
    labels: List[str],
    dataset: str,
    output_dir: str = "./results",
    ...
    """
    Bar chart showing per-task variance/std across permutations for each method.

    This helps visualize which tasks are most sensitive to task ordering.
    """
    if not results_list or not results_list[0]["results"]:
        print("Warning: No results to plot for taskwise variance")
        return None

    num_tasks = len(results_list[0]["results"][0]["accuracies"])
    num_methods = len(labels)

    # Compute per-task std for each method
    task_stds = []
    for result in results_list:
        acc_matrix = np.array([r["accuracies"] for r in result["results"]])
        # Std across permutations for each task
        stds = np.std(acc_matrix, axis=0)
        task_stds.append(stds)

    task_stds = np.array(task_stds)  # Shape: (num_methods, num_tasks)
    ...
    colors = [COLORS["er"]] + HIERARCHY_COLORS[:num_methods-1]

    for i, (label, stds) in enumerate(zip(labels, task_stds)):
        offset = (i - num_methods / 2 + 0.5) * width
        bars = ax.bar(x + offset, stds, width, label=label,
    ...
    ax.set_title(f"Task-wise Variance Across Permutations on {dataset}",
                fontweight='bold', fontsize=14)
    ax.set_xticks(x)
    ...
    max_std = np.max(task_stds) if task_stds.size > 0 else 10
    ax.set_ylim(0, max_std + 5)

    plt.tight_layout()
    save_figure(fig, f"taskwise_variance_{dataset}", output_dir)
...
def plot_taskwise_variance_heatmap(
    results_list: List[Dict[str, Any]],
    labels: List[str],
    dataset: str,
    output_dir: str = "./results",
    ...
    """
    Heatmap showing per-task variance for each method.

    Rows: Methods, Columns: Tasks
    Color intensity: Variance (higher = more sensitive to task order)
    """
    if not results_list or not results_list[0]["results"]:
        print("Warning: No results to plot for taskwise variance heatmap")
        return None
    ...
    for result in results_list:
        acc_matrix = np.array([r["accuracies"] for r in result["results"]])
        stds = np.std(acc_matrix, axis=0)
        variance_matrix.append(stds)

    variance_matrix = np.array(variance_matrix)

    # Create figure - adjust size based on number of methods
    fig_height = max(4, len(labels) * 0.5 + 2)
    ...
    sns.heatmap(
        variance_matrix,
        ax=ax,
        cmap="YlOrRd",  # Yellow-Orange-Red: higher variance = more red
        annot=True,
        fmt=".1f",
    ...
    ax.set_title(f"Task-wise Variance Heatmap on {dataset}\n(Higher = More Sensitive to Task Order)",
                fontweight='bold', fontsize=12)

    plt.tight_layout()
    save_figure(fig, f"taskwise_variance_heatmap_{dataset}", output_dir)
...
# =============================================================================
# FIXED: Accuracy Heatmap with All Annotations
# =============================================================================
...
def plot_accuracy_heatmap(
    ...
    output_dir: str = "./results",
    show: bool = False,
    max_permutations: int = 30,  # Limit for readability
) -> plt.Figure:
    """
    Heatmap showing accuracy for each permutation and task.

    FIXED: Now properly shows annotations for ALL rows by:
    1. Dynamically adjusting figure size based on number of permutations
    2. Adjusting font size for annotations
    3. Limiting permutations if too many for readability
    """
    # Prepare data
    acc_matrix = np.array([r["accuracies"] for r in results["results"]])
    num_perms, num_tasks = acc_matrix.shape

    # Limit permutations if too many (for readability)
    if num_perms > max_permutations:
        print(f"Note: Showing first {max_permutations} of {num_perms} permutations for readability")
        acc_matrix = acc_matrix[:max_permutations]
        num_perms = max_permutations

    # Calculate dynamic figure size
    # Height: at least 0.4 inches per row, minimum 6 inches
    fig_height = max(6, num_perms * 0.4)
    # Width: proportional to number of tasks
    fig_width = max(8, num_tasks * 1.5 + 2)

    fig, ax = plt.subplots(figsize=(fig_width, fig_height))

    # Calculate font size for annotations based on number of rows
    ...
    # Create heatmap with explicit annotation settings
    heatmap = sns.heatmap(
        acc_matrix,
        ax=ax,
        cmap="RdYlGn",
        ...
        yticklabels=[f"P{i+1}" for i in range(num_perms)],
        vmin=0,
        vmax=100,
    ...
    # Ensure all y-tick labels are shown
    ax.set_yticks(np.arange(num_perms) + 0.5)
    ax.set_yticklabels([f"P{i+1}" for i in range(num_perms)], rotation=0)

    ax.set_xlabel("Task", fontweight='bold', fontsize=12)
    ax.set_ylabel("Permutation", fontweight='bold', fontsize=12)
...
def create_all_visualizations(
    ...
    # Prepare labels and results list for multi-method plots
    ...
    # 5. Forgetting comparison
    plot_forgetting_comparison(results_list, labels, dataset, output_dir, show)

    # 6. Taskwise variance plots
    plot_taskwise_variance(results_list, labels, dataset, output_dir, show)
    plot_taskwise_variance_heatmap(results_list, labels, dataset, output_dir, show)

    # 7. Comprehensive comparison
    plot_comprehensive_comparison(er_results, htcl_results_by_level, dataset, output_dir, show)

    # 8. Heatmaps (FIXED - now shows all annotations)
