...
def global_catchup(
    global_model: nn.Module,
    recent_loaders: List[DataLoader],
    num_iterations: int,
    device: str,
    eta: float = 0.1,
    max_norm: float = 1.0,
    ...
    Args:
        global_model: Global model to update
        recent_loaders: DataLoaders for recent tasks
    ...
    for seq_id, perm in enumerate(perms, start=1):
        ...
        for g_idx, task_group in enumerate(task_groups):
            ...
            if catchup_enabled and g_idx > 0:
                recent_loaders = group_train[-min(2, len(group_train)):]
                global_catchup(
                    hierarchy.global_model,
                    recent_loaders,
                    num_iterations=catchup_epochs,
                    device=device,
                    eta=eta * 2.0,  # More aggressive for catch-up
                    max_norm=max_norm,
                    lambda_reg=500.0,  # Lower reg = move more toward local
                    verbose=verbose
                )
