...
IMPORTANT: samples_per_class should be passed from config.
The default value here (None) means "use full dataset".
"""
...
class BaseContinualDataset(ABC):
    ...
    def __init__(
        ...
        seed: int = 42,
        debug: bool = False,
        samples_per_class: Optional[int] = None,  # None = use full dataset
        num_workers: int = 2,
    ):
        """
        ...
        Args:
            ...
            debug: If True and samples_per_class is set, limit samples
            samples_per_class: Max samples per class (None = full dataset)
                              This should come from config.DEBUG_SAMPLES_PER_CLASS
    ...
    @property
    def should_limit_samples(self) -> bool:
        """Check if we should limit samples (debug mode with samples_per_class set)."""
        return self.debug and self.samples_per_class is not None
...
class RemappedSubset(Dataset):
    """Dataset wrapper that remaps labels for task-specific classification."""
